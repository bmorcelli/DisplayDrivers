...
size_t tft_display::write(uint8_t c) { return TFT_eSPI::write(c); }

size_t tft_display::write(const uint8_t *buffer, size_t size) {
    // TFT_eSPI's write(uint8_t) hides Print's buffer overload.
    return buffer ? Print::write(buffer, size) : 0;
}

int16_t tft_sprite::fontHeight(int16_t font) const {
    return const_cast<tft_sprite *>(this)->TFT_eSprite::fontHeight(font);
}
