...
void tft_display::begin(uint32_t speed) {
    ...
    // Panel_sdl is a framebuffer, not a bus device: no SPI/parallel wiring to
    // configure, just the geometry the window opens at. displayConfig is what
    // the harness's _setup_gpio()-equivalent is expected to edit (tftWidth /
    // tftHeight) before calling begin(), the same way a board's DisplayConfig
    // override works on every other backend.
    auto cfg = _panel_instance.config();
    cfg.memory_width = displayConfig.width;
    cfg.panel_width = displayConfig.width;
    cfg.memory_height = displayConfig.height;
...
void tft_display::drawEllipse(int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color) {
    lgfx::LGFX_Device::drawEllipse(x0, y0, rx, ry, color);
}

void tft_display::fillEllipse(int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color) {
    lgfx::LGFX_Device::fillEllipse(x0, y0, rx, ry, color);
}
...
void tft_display::drawXBitmap(
    ...
    if (!bitmap) return;
    lgfx::LGFX_Device::drawXBitmap(x, y, bitmap, w, h, color);
}
...
void tft_display::drawXBitmap(
    ...
    if (!bitmap) return;
    lgfx::LGFX_Device::drawXBitmap(x, y, bitmap, w, h, color, bg);
}
...
void tft_display::invertDisplay(bool i) { lgfx::LGFX_Device::invertDisplay(i); }
...
int16_t tft_display::textWidth(const char *s, uint8_t font) const {
    (void)font;
    if (!s) return 0;
    return static_cast<int16_t>(strlen(s) * 6 * (_textSize ? _textSize : 1));
}
...
void tft_display::setTextWrap(bool wrapX, bool wrapY) { lgfx::LGFX_Device::setTextWrap(wrapX, wrapY); }
...
static SPIClass s_dummySpi;
SPIClass &tft_display::getSPIinstance() const { return s_dummySpi; }
...
lgfx::LGFX_Device *tft_display::native() { return this; }

int16_t tft_display::drawAlignedString(const String &s, int32_t x, int32_t y, uint8_t datum) {
    int16_t w = static_cast<int16_t>(s.length() * 6 * (_textSize ? _textSize : 1));
    int16_t h = static_cast<int16_t>(8 * (_textSize ? _textSize : 1));
    int32_t cx = x;
    int32_t cy = y;
    switch (datum) {
    ...
    lgfx::LGFX_Device::print(s.c_str());
    return w;
}
...
int16_t tft_sprite::drawString(const String &string, int32_t x, int32_t y, uint8_t font) {
    (void)font;
    lgfx::LGFX_Sprite::setCursor(x, y);
    lgfx::LGFX_Sprite::print(string.c_str());
    return static_cast<int16_t>(string.length() * 6);
}
