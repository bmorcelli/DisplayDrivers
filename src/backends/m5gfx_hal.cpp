...
void tft_display::drawArc(
    ...
    (void)bg_color;
    (void)smoothArc;
    RUN_ON_MUTEX(M5_DRV.fillArc(x, y, r, ir, startAngle + 90, endAngle + 90, (uint16_t)fg_color));
}
...
void tft_display::drawWideLine(
    ...
    (void)bg_color;
    RUN_ON_MUTEX(M5_DRV.drawWideLine(ax, ay, bx, by, wd, (uint16_t)fg_color));
}
...
void tft_display::drawXBitmap(
    ...
    if (!bitmap) return;
    RUN_ON_MUTEX(M5_DRV.drawXBitmap(x, y, bitmap, w, h, (uint16_t)color));
}
...
void tft_display::drawXBitmap(
    ...
    if (!bitmap) return;
    RUN_ON_MUTEX(M5_DRV.drawXBitmap(x, y, bitmap, w, h, (uint16_t)color, (uint16_t)bg));
}

void tft_display::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, const uint16_t *data) {
    RUN_ON_MUTEX(M5_DRV.pushImage(x, y, w, h, data));
}
...
void tft_display::pushImage(
    ...
    for (int32_t row = 0; row < h; ++row) {
        for (int32_t col = 0; col < w; ++col) {
            ...
            RUN_ON_MUTEX(M5_DRV.drawPixel(x + col, y + row, (uint16_t)color));
        }
    }
}
...
void tft_display::setCursor(int16_t x, int16_t y) { M5_DRV.setCursor(x, y); }

int16_t tft_display::getCursorX() const { return M5_DRV.getCursorX(); }

int16_t tft_display::getCursorY() const { return M5_DRV.getCursorY(); }

void tft_display::setTextSize(uint8_t s) {
    _textSize = s ? s : 1;
    M5_DRV.setTextSize(_textSize);
}

void tft_display::setTextColor(uint16_t c) {
    _textColor = c;
    M5_DRV.setTextColor((uint16_t)c);
}
...
void tft_display::setTextColor(uint16_t c, uint16_t b, bool bgfill) {
    ...
    _textColor = c;
    _textBgColor = b;
    M5_DRV.setTextColor((uint16_t)c, (uint16_t)b);
}
...
size_t tft_display::write(uint8_t c) {
    size_t t = 0;
    RUN_ON_MUTEX(t = M5_DRV.write(c));
    return t;
}

size_t tft_display::write(const uint8_t *buffer, size_t size) {
    size_t t = 0;
    RUN_ON_MUTEX(t = M5_DRV.write(buffer, size));
    return t;
}

size_t tft_display::println() {
    size_t t = 0;
    RUN_ON_MUTEX(t = M5_DRV.println());
    return t;
}
...
size_t tft_display::printf(const char *fmt, ...) {
    ...
    if (static_cast<size_t>(len) < sizeof(stackBuf)) {
        RUN_ON_MUTEX(t = M5_DRV.print(stackBuf));
        return t;
    }
    ...
    va_end(args);
    RUN_ON_MUTEX(t = M5_DRV.print(buf.get()));
    return t;
}

int16_t tft_display::width() const { return M5_DRV.width(); }

int16_t tft_display::height() const { return M5_DRV.height(); }
...
#endif
#if defined(USE_CANVAS)
    RUN_ON_MUTEX(_canvas.pushSprite(0, 0));
#endif
}
...
int16_t tft_display::drawAlignedString(const String &s, int32_t x, int32_t y, uint8_t datum) {
    ...
    M5_DRV.setCursor(cx, cy);
    RUN_ON_MUTEX(M5_DRV.print(s));
    return w;
}
