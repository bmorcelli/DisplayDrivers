...
#ifndef LIB_HAL_EPDPAINTER_H
#define LIB_HAL_EPDPAINTER_H
#include <pins_arduino.h>

#if defined(USE_EPD_PAINTER)

// EPD_Painter_presets.h must be included first: it resolves EPD_PAINTER_PRESET
// from the EPD_PAINTER_PRESET_* macro (or from the AUTO probe table).
...
// clang-format off
// ---------------------------------------------------------------------------
// Configuration macros (all optional — the defaults match a 4-level parallel
// panel driven through the preset selected in EPD_Painter_presets.h)
// ---------------------------------------------------------------------------

// Build the canvas in portrait (the panel presets are declared landscape) and
// let setRotation() pick the view, matching the rest of the HAL.
#ifndef EPD_PAINTER_PORTRAIT
    #define EPD_PAINTER_PORTRAIT 1
#endif

// Waveform quality: QUALITY_FAST, QUALITY_NORMAL or QUALITY_HIGH.
#ifndef EPD_PAINTER_QUALITY
    #define EPD_PAINTER_QUALITY QUALITY_HIGH
#endif

// Grey levels the canvas is indexed against: 4 (default) or 16.
#ifndef EPD_PAINTER_GREY_LEVELS
    #define EPD_PAINTER_GREY_LEVELS 4
#endif
#if EPD_PAINTER_GREY_LEVELS != 4 && EPD_PAINTER_GREY_LEVELS != 16
    #error "EPD_PAINTER_GREY_LEVELS must be 4 or 16"
#endif

// Bounding-box tolerance (px) used to merge dirty rects before each paint.
#ifndef EPD_PAINTER_DIRTY_TOLERANCE
    #define EPD_PAINTER_DIRTY_TOLERANCE 10
#endif
// clang-format on
...
public:
    ...
    EPD_PainterAdafruit *native();

    // --- e-paper specific -------------------------------------------------
    // Flush the canvas to the panel. fullRefresh forces a hard clear of the
    // whole panel first; otherwise only the areas that changed are cleared.
    void display(bool fullRefresh = false);

    // Driving a parallel EPD is bit-banged and timing sensitive. Register the
    // handle of a task that must be suspended while the panel is being driven
    // (a pointer, so the guard follows the handle being cleared at runtime).
    void setPaintGuard(TaskHandle_t *handle);

    // Panel configuration, including the I2C bus the preset brought up — board
    // code needs it to reach the touch controller / PMIC sharing that bus.
    const EPD_Painter::Config &getConfig() const;
    EPD_Painter *driver();
...
private:
    template <typename Ptr> void pushImageFallback(int32_t x, int32_t y, int32_t w, int32_t h, Ptr data) {
        ...
        for (int32_t row = 0; row < h; ++row) {
            for (int32_t col = 0; col < w; ++col) {
                ...
                _gfx->drawPixel(x + col, y + row, mapColor(color));
            }
        }
    }
    ...
    // RGB565 -> canvas level code (0 = paper white … LEVELS-1 = black).
    static uint8_t mapColor(uint16_t color);

    EPD_PainterAdafruit *_gfx = nullptr;
    TaskHandle_t *_paintGuard = nullptr;
