...
public:
...
#endif
    // TFT_eSPI wants the font spelled out every time; the rest of the
    // backends default it, so these wrappers keep callers portable.
    ...
    // TFT_eSPI has no panel sleep of its own (the command is driver specific),
    // so this is a no-op, same as the Arduino_GFX backend.
